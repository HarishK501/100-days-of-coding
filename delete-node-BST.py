...
def deleteNodeBST(root, data):
    if root:
        if root.data == data:
            if not root.left:
                return root.right   # here if root.right also does not exist, then the node to be deleted is a leaf node. So None will be returned here
            ...
            # if the node to be deleted contains both child nodes
                ...
                root.data = fullNodeInorderSuccessor(root).data
